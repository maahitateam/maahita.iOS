...
//  maahita
//
//  Created by Jagan Kumar Mudila on 22/07/2020.
...
class CompletedSessionsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        flowLayout.itemSize = CGSize(width: self.view.bounds.width-20.0, height: 120.0)
    ...
    func updateUI() {
        self.viewModel.fetchCompletedSessions()
    }
}

extension CompletedSessionsViewController: UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        
        guard let cell = collectionView.cellForItem(at: indexPath) as? SessionCell else { return }
        
        let sessionViewController = SessionViewController(sessionID: cell.maahitaSession.id, groupID: nil)
        self.present(sessionViewController, animated: true) {
            collectionView.deselectItem(at: indexPath, animated: true)
        }
    }

    func CompletedSessionsViewController(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        return CGSize(width: collectionView.bounds.width, height: collectionView.bounds.height * 0.35)
    }
}

extension CompletedSessionsViewController: AuthServiceDelegate {
    func refreshUser() {
        self.updateUI()
    }
}
